...
case $ACTION in
    ...
    "jupyter")
        ...
        if systemctl is-active jupyterhub >/dev/null 2>&1; then
            ...
            echo "sourcing ~/.bashrc and looking for CONDA_EXE"
            source $HOME/.bashrc
            ...
            if [ -n "${CONDA_EXE:-}" ]; then
                ...
                CONDADIR=$(find $HOME -type d -wholename "*/miniconda3" 2>/dev/null -print -quit)
                CONDAINIT="${CONDADIR}/etc/profile.d/conda.sh"
                if [ -f "$CONDAINIT" ]; then
